...
impl PopulationApi {
    ...
    pub fn new(pyconfig: Py<PyDict>) -> PyResult<Self> {
        let population = Python::with_gil(|py| -> PyResult<Population> {
            ...
            let from_scratch: bool = match config.get_item("evolve_from_scratch")? {
                Some(flag) => flag.extract()?,
                None => false,
            };

            let module_name: String = network_module.unwrap();

            let qdm: QDManager = if from_scratch {
                ...
                QDManager::new(Arc::from(module_name), Some(py_url.url))
            };
    ...
    /// used as the starting point for evolution.
    pub fn agent_post(&self, idx: usize, file_path: PathBuf) -> PyResult<()> {
