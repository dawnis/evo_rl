...
impl PopulationApi {
    ...
    pub fn new(pyconfig: Py<PyDict>, endpoint: Option<Url>) -> PyResult<Self> {
        let population = Python::with_gil(|py| -> PyResult<Population> {
            ...
            let qdm: QDManager = match endpoint {
                Some(api) => {
                    let module_str = network_module.unwrap();
                    QDManager::new(Arc::from(module_str), endpoint).await()
                }
                None => {
                    let genome_base = EneCode::new(
                        input_size,
                        hidden_size,
                        output_size,
                        network_module.as_deref(),
                    );
                    QDManager::new_from_genome(network_module, endpoint, genome_base)
                }
            };
